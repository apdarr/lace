class PlanPhotoProcessor
  ...
  def process_single_photo(photo)
    ...
    Rails.logger.debug "PlanPhotoProcessor: GPT response: #{result}"
    ...
    begin
      ...
      Rails.logger.error "PlanPhotoProcessor: GPT response was: #{result}"
    end
  end

  def create_activities_from_workouts(workouts)
